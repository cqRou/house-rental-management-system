...
public class StringUtils {
     ...
     * @since 0.4
     */
    public static boolean contains(String sourceString, String targetString) {
     ...
     * @since 0.4
     */
    public static boolean containsIgnoreCase(List<String> strings, String string) {
    ...
    public static String combineStringList(List<String> array) {
        if (array == null)
            return "";
        StringBuilder result = new StringBuilder();
        for (String aArray : array) {
            result.append(aArray).append(",");
        }
        if (!StringUtils.isBlank(result.toString()) && result.toString().length() >= 1) {
     ...
     * @return 最终生成的字符串
     */
    public static String fillLeftWithChar(String str, char c, int length) {
    ...
    public static boolean isHttp(String http) {
        return !isNULL(http) && http.length() >= 2
    ...
    public static Date toDate(String date) throws ParseException {
        ...
        if (date.length() > 16 && StringUtils.countMatches(date, "-") == 2
                && StringUtils.countMatches(date, ":") == 2) {
            fomat = DateUtils.FULL_ST_FORMAT;
        } else if (date.length() > 12
                && StringUtils.countMatches(date, "-") == 2
                && StringUtils.countMatches(date, ":") == 1) {
            fomat = DateUtils.CURRENCY_ST_FORMAT;
        } else if (date.length() > 16
                && StringUtils.countMatches(date, "/") == 2
                && StringUtils.countMatches(date, ":") == 2) {
            fomat = DateUtils.FULL_J_FORMAT;
        } else if (date.length() > 12
                && StringUtils.countMatches(date, "/") == 2
                && StringUtils.countMatches(date, ":") == 1) {
            fomat = DateUtils.CURRENCY_J_FORMAT;
            ...
            fomat = "yy年MM月dd日HH:mm:ss";
                ...
                && date.indexOf("日") != -1
                && StringUtils.countMatches(date, ":") == 1) {
            fomat = DateUtils.CN_FORMAT;
        } else if (date.length() > 8 && date.length() < 10
                && StringUtils.countMatches(date, "-") == 2) {
        } else if (date.length() > 5 && date.length() < 9
                && StringUtils.countMatches(date, "-") == 2) {
            fomat = DateUtils.SHORT_DATE_FORMAT;
        }
        if (!StringUtils.isNULL(fomat)) {
    ...
    /**
     * 格式化bool字符串为Y/N；如1，yes，YES，true等返回Y
     *
     * @param targetValue
     * @return
     */
    public static String formatBoolToYesNo(String targetValue) {
        if ("1".equals(targetValue) || "y".equalsIgnoreCase(targetValue)
                || "yes".equalsIgnoreCase(targetValue)
                || "t".equalsIgnoreCase(targetValue)
                || "true".equalsIgnoreCase(targetValue)) {
            return "Y";
        } else {
            return "N";
        }
     ...
     * @return
     */
    public static String join(String[] params, String splitTag) {
    ...
    public static String join(List<String> params, String splitTag) {
        StringBuffer str = new StringBuffer();
    ...
    public static boolean isNumberWithDot(String numStr) {
        if (isNULL(numStr))
            return false;
        String regStr = "^[1-9][0-9]*$|^[1-9][0-9]*.?[0-9]*$";
    ...
    public static String randomNumberStr(int numberLen) {
        //先产生随机数, 位数 numberLen
        int range = ((int) Math.pow(10, numberLen));
        java.util.Random random = new java.util.Random();
        long num = random.nextInt(range);
    ...
    public static void main(String args[]) {
        ...
        userlist.add("entp_admin");
        System.out.println(StringUtils.containsIgnoreCase(userlist, com.heeexy.example.util.Constant.ROLE_ENTP_PREF));
    }
